...
/** @typedef {import('@advanced-rest-client/arc-types').ArcResponse.ResponseAuth} ResponseAuth */
...
export class BaseRequest extends EventEmitter {
   ...
   */
  get timeout() {
    const { arcRequest, opts } = this;
    if (arcRequest.config && typeof arcRequest.config.timeout === 'number') {
      return arcRequest.config.timeout;
    }
  ...
  get followRedirects() {
    const { arcRequest, opts } = this;
    if (arcRequest.config && typeof arcRequest.config.followRedirects === 'boolean') {
      return arcRequest.config.followRedirects;
    }
...
Check your request parameters.`);
  ...
  _computeStats(stats) {
    ...
    let receive =
      lastReceivedTime && firstReceiveTime ? lastReceivedTime - firstReceiveTime : -1;
    if (receive < 0) {
      receive = 0;
    }
  ...
  _addClientCertificate(certificate, options) {
    ...
    if (cert.type === 'p12') {
      ...
      options.cert = cert.cert.map((item) => Buffer.from(item.data));
      if (cert.key) {
