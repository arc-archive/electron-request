...
export class ElectronRequest extends BaseRequest {
  ...
  async send() {
    ...
    const request = this.opts.proxy ? this._connectProxy(message) : this._connect(message);
   ...
   * @return {http.ClientRequest}
   */
  _connectProxy(message) {
    const { proxy, proxyAuth } = this.opts;
    ...
    if (!isProxySsl && !isTargetSsl) {
      return this._proxyHttpOverHttp(message, uri, proxy, proxyAuth);
    }
    if (!isProxySsl && isTargetSsl) {
      return this._proxyHttpsOverHttp(message, uri, proxy, proxyAuth);
    }
    if (isProxySsl && !isTargetSsl) {
      return this._proxyHttpOverHttps(message, uri, proxy, proxyAuth);
    }
    return this._proxyHttpsOverHttps(message, uri, proxy, proxyAuth);
  }
   ...
   * @param {string=} auth Optional authorization.
   * @return {http.RequestOptions}
   */
  _createProxyOptions(proxy, requestUri, requestOptions, auth) {
    let proxyUrl = proxy;
    ...
    if (auth) {
      const hash = `Basic ${Buffer.from(auth).toString('base64')}`;
      if (!options.headers['proxy-authorization']) {
        options.headers['proxy-authorization'] = hash;
      }
    }
   ...
   * @param {string=} auth Optional authorization header value (not encoded).
   * @return {http.ClientRequest}
   */
  _proxyHttpOverHttp(message, uri, proxy, auth) {
    const targetOptions = this._createGenericOptions(uri);
    const options = this._createProxyOptions(proxy, uri, targetOptions, auth);
    const startTime = Date.now();
    this.stats.startTime = startTime;
    this.transportRequest.startTime = startTime;
    const request = http.request(options);
   ...
   * @return {http.ClientRequest}
   */
  _proxyHttpsOverHttp(message, uri, proxy, auth) {
    let proxyUrl = proxy;
   ...
   * @return {http.ClientRequest}
   */
  _proxyHttpOverHttps(message, uri, proxy, auth) {
    const targetOptions = this._createGenericOptions(uri);
    const options = /** @type https.RequestOptions */ (this._createProxyOptions(proxy, uri, targetOptions, auth));
    options.rejectUnauthorized = false;
    // @ts-ignore
    options.requestOCSP = false;
    const startTime = Date.now();
   ...
   * @return {http.ClientRequest}
   */
  _proxyHttpsOverHttps(message, uri, proxy, auth) {
    let proxyUrl = proxy;
