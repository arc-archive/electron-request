...
const {HostRulesEval} = require('./hosts-eval');
...
class SocketRequest extends EventEmitter {
  ...
  prepareMessage() {
    ...
    .then((buffer) => {
      this._addContentLength(buffer, headers);
  ...
  writeMessage(buffer) {
    ...
    this.arcRequest.sentHttpMessage = buffer.toString();
   ...
   * needed
   */
  _addContentLength(buffer, headers) {
  ...
  _createResponse(opts) {
    ...
    if (opts.error) {
      const resp = {
        ...
        stats: this._cleanupStats(this.stats)
      };
   ...
   */
  _cleanupStats(stats) {
    let {connect, receive, send, wait, dns, ssl} = stats;
    const type = 'number';
    if (typeof connect !== type) {
      connect = -1;
    }
    if (typeof receive !== type) {
      receive = -1;
    }
    if (typeof send !== type) {
      send = -1;
  ...
  _errorRequest(opts) {
    ...
    this.emit('error', this.id, error, this.arcRequest, response);
    this._cleanUp();
  }
}
