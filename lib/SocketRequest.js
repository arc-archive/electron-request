...
/** @typedef {import('./RequestTypes.js').LegacyAuth} LegacyAuth */
/** @typedef {import('./RequestTypes.js').LegacyAuth} NtlmAuthorization */
/** @typedef {import('./RequestTypes.js').LegacyAuth} NtlmAuthLegacy */
/** @typedef {import('./RequestTypes.js').ArcRequest} ArcRequest */
...
export class SocketRequest extends BaseRequest {
   ...
   *
   * @param {ArcRequest} request ARC's request object
  ...
  async prepareMessage() {
    ...
    const message = await this._prepareMessage(buffer);
    if (this.auth) {
  ...
  _handleAuthorization(headers) {
    const { auth, authType } = this.arcRequest;
    if (!auth) {
      return;
    }
    const legacyAuth = /** @type LegacyAuth */ (auth);
    if (!authType && legacyAuth.method) {
      this._handleLegacyAuth(headers, legacyAuth);
      return;
    }
    if (authType === 'ntlm') {
      const data = /** @type NtlmAuthorization */ (auth);
      this._authorizeNtlm(data, headers);
    }
  }
  ...
  handleNtlmResponse() {
    if (this.auth.state === 0) {
      if (this._response._headers.has('www-authenticate')) {
        this.auth.state = 1;
        this.auth.challengeHeader = this._response._headers.get(
          'www-authenticate'
        );
  ...
  _processStatus(data) {
    ...
    this._response.status = typedStatus;
  ...
  _processHeaders(data) {
    ...
    if (index === -1 && enterIndex !== 0) {
      ...
      if (!this._rawHeaders) {
        ...
        const sum = this._rawHeaders.length + data.length;
    ...
    if (enterIndex !== 0) {
      ...
      if (!this._rawHeaders) {
        ...
        const sum = this._rawHeaders.length + headersArray.length;
  ...
  _postHeaders(data) {
    ...
    if (data.length === 0) {
      if (this._response._headers.has('Content-Length')) {
        ...
        const length = Number(this._response._headers.get('Content-Length'));
        ...
        !this._response._headers.has('Transfer-Encoding') ||
  ...
  _processBodyChunked(data) {
    ...
    while (true) {
      ...
      const size = Math.min(this._chunkSize, data.length);
