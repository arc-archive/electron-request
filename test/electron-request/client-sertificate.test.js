...
describe.only('Client certificate', function() {
  const httpPort = 8345;
  ...
  const opts = [{
    ...
    cert: {
      cert: fs.readFileSync('./test/cert-auth-server/alice_cert.pem', 'utf8'),
      key: fs.readFileSync('./test/cert-auth-server/alice_key.pem', 'utf8'),
      type: 'pem',
    },
  }, {
    cert: {
      // has to be buffer
      cert: fs.readFileSync('./test/cert-auth-server/alice.p12'),
      type: 'p12',
      passphrase: '',
    },
  }, {
    cert: {
      // has to be buffer
      cert: fs.readFileSync('./test/cert-auth-server/alice-password.p12'),
      type: 'p12',
      passphrase: 'test',
    },
  }, {
    cert: {
      // has to be buffer
      cert: fs.readFileSync('./test/cert-auth-server/bob.p12'),
      type: 'p12',
      passphrase: 'test',
    },
  }];
